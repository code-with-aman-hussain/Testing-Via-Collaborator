...
export class q_expense_perdiem_dcl1 extends q_expense_perdiem_dcl1_gen {
  public async perdiem_list_caller(oEvent) {
    ...
    for (let i = 0; i < (await Alldata).length; i++) {
      ...
      if(Alldata[i].r_perdiem_travel[0].travel_type==="Local"){
        Alldata[i].transient_no_of_days = this.localPerdiemEligibleDays(
    ...
    await instance.setLoadAll(true);
  ...
  public async localPerdiemEligibleDays(
    ...
    for (const row of localdays) {
      ...
      if (day >= transient_start_date && day <= transient_end_date && day >=pd_start_date && day <=pd_end_date) {
        count++;
      }
    }
